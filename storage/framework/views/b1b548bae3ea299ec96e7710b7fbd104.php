...
<div class="p-4 sm:p-8 max-w-6xl mx-auto">
    <h1 class="text-3xl font-semibold"><?php echo e(__('ui.admin_procedures_calendar')); ?></h1>
    <p class="text-muted mt-2"><?php echo e(__('ui.admin_procedures_desc')); ?></p>

    <?php if(session('success')): ?>
        <div class="mt-4 flash-success"><?php echo e(session('success')); ?></div>
    <?php endif; ?>
                ...
                <form id="schedule-form" method="POST" class="mt-6 space-y-6">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="font-semibold"><?php echo e(__('ui.procedures')); ?></h3>
                            <button type="button" id="add-procedure" class="text-sm px-3 py-1 rounded-lg btn-violet">+ <?php echo e(__('ui.add')); ?></button>
                        </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="font-semibold"><?php echo e(__('ui.available_times')); ?></h3>
                            <button type="button" id="add-time" class="text-sm px-3 py-1 rounded-lg btn-violet">+ <?php echo e(__('ui.add')); ?></button>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-3 pt-4 border-t" style="border-color: rgb(var(--border));">
                        <button type="submit" class="px-5 py-3 rounded-xl btn-primary"><?php echo e(__('ui.apply_date')); ?></button>
                        <button type="button" id="apply-all-btn" class="px-5 py-3 rounded-xl border" style="border-color: rgb(var(--border));"><?php echo e(__('ui.apply_all')); ?></button>
                    </div>
                </form>

                <div id="apply-all-modal" class="hidden mt-6 p-4 rounded-2xl panel-muted">
                    <h4 class="font-semibold mb-2"><?php echo e(__('ui.are_you_sure')); ?></h4>
                    <p class="text-sm text-muted mb-4"><?php echo e(__('ui.apply_all_desc')); ?></p>
                    ...
                    <form id="apply-all-form" method="POST" class="grid sm:grid-cols-2 gap-3">
                        ...
                        <button type="submit" class="sm:col-span-2 px-5 py-3 rounded-xl btn-accent"><?php echo e(__('ui.confirm_apply_all')); ?></button>
                    </form>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const datesWithSchedule = <?php echo json_encode($datesWithSchedule->keys(), 15, 512) ?>;
    ...
    document.getElementById('apply-all-btn')?.addEventListener('click', () => {
        ...
        preview.innerHTML = `<p><strong><?php echo e(__('ui.procedures')); ?>:</strong> ${procLines.join(', ') || '-'}</p><p><strong><?php echo e(__('ui.available_times')); ?>:</strong> ${timeLines.join(', ') || '-'}</p>`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/lasma/Desktop/programming/priv/lashbook/resources/views/admin/procedures.blade.php ENDPATH**/ ?>